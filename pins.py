...
def check_io(): 
    ...
    IO_GROUP4[0].dir(mraa.DIR_OUT)       #GPIO.setup(IO_GROUP4[0], GPIO.OUT)
    for pin in IO_GROUP4[1:]:
        pin.dir(mraa.DIR_IN)     #GPIO.setup(pin, GPIO.IN)
    IO_GROUP4[0].write(0)        #GPIO.output(IO_GROUP4[0], GPIO.LOW)
    ...
    for pin in IO_GROUP4[1:]:
        if pin.read() != 0:      #if GPIO.input(pin) != GPIO.LOW:
            badio.append(io_group4[IO_GROUP4.index(pin)])
